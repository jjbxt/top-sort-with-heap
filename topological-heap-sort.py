...
def compare_all(
    ...
    *,
    ...
    return {
        "Built-in Sort": sort_from_scratch_counted(items),
        "Topological Heapsort": topological_heapsort(
            items,
            known_less,
            use_transitive_known_for_stats=use_transitive_known_for_stats,
        ),
        "Topological Heapsort With Lookahead": topological_heapsort_with_lookahead(
            items,
            known_less,
            use_transitive_known_for_stats=use_transitive_known_for_stats,
        ),
...
def simple_example():
    ...
    known = [
        ...
        (6, 7),
        (6, 9),
        # chain creates singleton-ish bottleneck structure
        (2, 7),
        (4, 8),
        (1, 2),
        (5, 8),
...
def vary_n_benchmarks():
    res = {"Built-in Sort": [], "Topological Heapsort": [], "Topological Heapsort With Lookahead": []}

    ns = [100, 1000, 5000, 10_000, 20_000, 50_000, 100_000, 200_000, 400_000]  # , 100_000, 300_000, 500_000, 1_000_000]
    ...
    x, y = zip(*res["n log n"])
    plt.plot(x, y, label="n log n", linestyle="dashed")

    plt.xlabel("n")
    plt.ylabel("total item comparisons")
    plt.title("Total item comparisons vs n, where m = n")
    plt.legend()
    plt.show()


def vary_m_benchmarks():
    res = {
        "Built-in Sort": [],
        "Topological Heapsort": [],
        "Topological Heapsort With Lookahead": [],
    }

    n = 10_000
    ms = [5_000, 10_000, 20_000, 50_000, 100_000, 200_000, 500_000, 1_000_000, 2_000_000]
    seeds = range(124, 134)  # 10 different seeds
    ...
    for m in ms:
        ...
        for seed in seeds:
            ...
            results = compare_all(
                items,
                known,
                use_transitive_known_for_stats=False,
            )

            for name, stats in results.items():
                totals[name].append(stats.total_item_comparisons)

        for name in res:
            avg_comparisons = sum(totals[name]) / len(totals[name])
            res[name].append((m, avg_comparisons))

    for name, data in res.items():
        x, y = zip(*data)
        plt.plot(x, y, label=name)

    plt.xlabel("m (number of known comparisons)")
    plt.ylabel("total item comparisons")
    plt.title("Total item comparisons vs m (known comparisons), where n = 10000")
    plt.legend()
    plt.show()
